...
let currentIndex = window.localStorage.getItem('currentIndex') || 0;

renderMenuItem();//调用左侧菜单
renderTodos();//生成右侧的todo(头部(内部有函数包含右侧容器内容))

// 主体结构*根据todolist的数据来生成保存menuItem(左侧菜单)
...
$('#addTodoInp').on('keyup', function (e) {
...
$('#addTodo').click(function () {
  ...
  // let todoObj=$('{title:"new",isLocked: false,todos: []}')
...
//点击右侧头部文字，变成可编辑状态，点击回车更改头部值title-input
$('#title').click(function () {
  //每一个当前右侧头部文本值给到右侧文本隐藏的input;
  $('#title-input').val($(this).html())  
  //头部显示部分
  $('.titleBox').css('display','none')
  //头部隐藏编辑部分的input
  $('.header .title-editer').css('display','block')
  //右侧头部隐藏点击取消x
  $('#cancel').click(function () {
    $('.titleBox').css('display','block')
    $('.header .title-editer').css('display','none')
    $('.titleBox').css('display','flex')
  })
  //右侧头部隐藏input绑定键盘事件
  $('#title-input').on('keyup', function (ev) {
    ev = ev || window.event;
    if (!todolist[currentIndex].isLocked == true) {
      if (ev.keyCode == 13) {
        ...
        $('.titleBox').css('display','block')
        $('.header .title-editer').css('display','none')
        todolist[currentIndex].title = $('#title').html();
        $('.titleBox').css('display','flex')
